...
class VACOnlineASRProcessor(OnlineASRProcessor):
    ...
    def __init__(self, online_chunk_size, *a, vad_threshold=0.5, vad_min_silence_duration_ms=500, **kw):
    ...
    def process_iter(self):
        if self.is_currently_final:
            ...
            if self._debug_counter % 100 == 1:  # Print every 100th call
                print(f"no online update, only VAD {self.status} (buffer size: {self.current_online_chunk_buffer_size}/{self.SAMPLING_RATE * self.online_chunk_size})", file=self.logfile)
