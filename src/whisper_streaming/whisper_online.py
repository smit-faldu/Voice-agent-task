...
import time
import logging
from src.whisper_streaming.backends import WhisperTimestampedASR
...
def add_shared_args(parser):
    ...
    # Backend is fixed to whisper_timestamped; argument removed to simplify configuration
    parser.add_argument(
        "--vac",
        action="store_true",
        default=False,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.4,
        help="The threshold for voice activity detection.",
    )
    parser.add_argument(
        "--vad-min-silence-duration-ms",
        type=int,
        default=500,
        help="The minimum silence duration in milliseconds for VAD.",
    )
    parser.add_argument(
        "--buffer_trimming",
        type=str,
...
def backend_factory(args):
    # Only whisper_timestamped backend is supported now
    size = args.model
    t = time.time()
    logger.info(f"Loading Whisper {size} model for {args.lan} using backend 'whisper_timestamped'...")
    asr = WhisperTimestampedASR(
        modelsize=size,
        lan=args.lan,
